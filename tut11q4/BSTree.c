...
void BSTreeLevelOrder(BSTree t)
{
    ...
    while (t != NULL) {
        ...
        showBSTreeNode(t);
        if (t->left != NULL){
            QueueJoin(newthing,t->left);
        }
        if (t->right != NULL){
            QueueJoin(newthing,t->right);
        }
        if(QueueIsEmpty(newthing) == 1){
			return;
		}
        t = QueueLeave(newthing);
    }
...
//#########################################################
BSTree insertRebalance(BSTree t, int it)
{
	t = BSTreeInsert(t, it);
	//float lsize = size(t->left), rsize = size(t->right);
    float lsize = BSTreeNumNodes(t->left), rsize = BSTreeNumNodes(t->right);
...
{
    printf("Called rebalance\n");
	if (t == NULL) return NULL;
	if (BSTreeNumNodes(t) < 2) return t;
	// put node with median key at root
	t = partition(t, BSTreeNumNodes(t)/2);
...
// move i'th element to root
BSTree partition(BSTree t, int i)
{
    printf("Called Partition\n");
	if (t == NULL) return NULL;
	assert(0 <= i && i < BSTreeNumNodes(t));
	int n = BSTreeNumNodes(t->left);
	if (i < n) {
